...
''' <summary>
''' A set of functions to validate user input.
''' </summary>
...
#Region "Date Validation"
    ...
    ''' <summary>
    ''' Loops through an array of known bank holidays and returns true if the selected date is a Bank Holiday. Returns False if no match is found.
    ''' </summary>
    ''' <param name="dateSelected"></param>
    ''' <returns>Boolean</returns>
    ...
    ''' <summary>
    ''' Checks to see if the start date precedes the end date or is the same as the end date. Returns boolean on the result.
    ''' </summary>
    ''' <param name="startDate"></param>
    ''' <param name="endDate"></param>
    ''' <returns>Boolean</returns>
    Public Function TestStartDatePrecedesEndDate(ByVal startDate As Date, ByVal endDate As Date) As Boolean
        Dim startDatePrecedesEndDate As Boolean

        If startDate <= endDate Then
            startDatePrecedesEndDate = True
        Else
            startDatePrecedesEndDate = False
        End If

        Return startDatePrecedesEndDate
    End Function
...
#Region "String Validation"
    ...
    ''' <summary>
    ''' Uses Regular Expression to validate the user has specified a valid email address. Returns a boolean on the result.
    ''' </summary>
    ''' <param name="input"></param>
    ''' <returns>Boolean</returns>
    Public Function Validate_Email(ByVal input As String) As Boolean
        Dim isValidEmail As Boolean = False
        Dim userInput As String = LCase(input)

        Dim myMatch As Match = System.Text.RegularExpressions.Regex.Match(userInput, “^[a-zA-Z][\w\.-]*[a-zA-Z0-9]@[a-zA-Z0-9][\w\.-]*[a-zA-Z0-9]\.[a-zA-Z][a-zA-Z\.]*[a-zA-Z]$”)
